...
let view_colorize_button =
  ...
  Vdom.Node.widget
    ...
    ~init:(fun () ->
      ...
      Js.Unsafe.fun_call colorify [| Js.Unsafe.inject button; Js.Unsafe.inject (Js.array (convert_lst_to_arr (Intro.simplification_list "x+1+2+3+4"))) |] |> ignore;
      (), button)
    ()
;;

let view _ ~inject:_ =
  Incr.return (Vdom.Node.div [] [ view_colorize_button;Vdom.Node.div [Vdom.Attr.id "graph-container"] [] ])
;;
