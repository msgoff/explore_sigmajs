...
module Model = struct
  ...
  let testModelFunction counter submitted_text = 
   { counter; input_text = sprintf "Default #%d" counter; submitted_text }
  ;;
  ...
  let render_graph t = testModelFunction 5 t.submitted_text 
...
(*
let adder =
    Js.Unsafe.fun_call ((Js.Unsafe.js_expr "window")##.adder) [|Js.Unsafe.inject (Js.string "NBN")|]
*)

let decodeURI =
  Js.Unsafe.fun_call (Js.Unsafe.js_expr "decodeURI") [|Js.Unsafe.inject (Js.string "NBN")|]

(*Firebug.console##log (Js.string "testmessage");*)

let test = 
Firebug.console##log (Js.Unsafe.fun_call (Js.Unsafe.js_expr "alert") [|Js.Unsafe.inject (Js.string "Test Alert")|]);
Firebug.console##log (Js.string "Test Console Log");
Js.Unsafe.fun_call (Js.Unsafe.js_expr "adder") [|Js.Unsafe.inject (Js.string "NBN")|]
;;  
...
let apply_action model action _ ~schedule_action:_ =
  ...
  | Render_graph -> Model.render_graph model
;;
...
let view (m : Model.t Incr.t) ~inject =
  ...
  let graph_button = button "Graph" Action.Render_graph in
  let%map input =
  ...
  in
  Node.body [] [ submission; input; submit_button; reset_button; incr_button; graph_button; Node.div [ Attr.id "graph-container"] []]
;;
